...
// NOTE: USE this custom 
// 162.55.106.16   chaosnet
// 162.55.106.17   testnet

const baseNodeIps = {
  testnet: process.env.TESTNET_NODE_IP || '162.55.106.17', // 'thorb.mctn.nexain.com',
  chaosnet: process.env.CHAOSNET_NODE_IP || '162.55.106.16', // 'thorb.mccn.nexain.com',
};
  ...
  try {
    ...
    if (rawdata) {
      ...
      if (sourceNode) {
        ...
        if (fiveMinsAgo.getTime() < sourceNode.timestamp) {
    ...
    if (response.data.indexOf(nodeIp) === -1) {
      nodeIp = response.data[Math.floor(Math.random() * response.data.length)];
...
export const thorchainDataFetcher = async (thorchainNetwork) => {
  ...
  const apiQueryForLongHistory = async (interval, subPeriod, baseUrl, getMeta) => {
    ...
    do {
      const url = `${baseUrl}interval=${interval}&from=${from}&count=300`;
  ...
  return {
    ...
    loadV1SinglechainNetwork: async () => {
      const nu = getSinglechainNetworkUrl(thorchainNetwork);
      const nd = await urlDataFetcher(nu)();
      return nd;
    },
