...
export function formatMoneyAmount(amount) {
  return numeral(amount).format('($0,00.0a)').toUpperCase();
}

export function poolNameWithoutAddr(poolName) {
  const poolNameParts = poolName.split('.');
  const firstPart = poolNameParts[0];
  const secondPart = poolNameParts[1];
  if (!secondPart) {
    return poolName;
  }
