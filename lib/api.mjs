...
export function mimirUrl() {
  return `${nodeBase}/thorchain/mimir`;
};

export function asgardVaultsUrl() {
  return `${nodeBase}/thorchain/vaults/asgard`;
};

export function poolAddressesUrl() {
...
export async function loadMimir({ axios }) {
  const url = mimirUrl();
  const { data } = await axios.get(url);
  return data;
}

export async function loadAsgardVaults({ axios }) {
  const url = asgardVaultsUrl();
  const { data } = await axios.get(url);
  return data;
}

export async function loadPoolAddresses({ axios }) {
  const url = poolAddressesUrl();
  const { data } = await axios.get(url);
  return data;
}
...
export async function loadNetwork({ axios }) {
