...
import axiosExt from 'axios-extensions';

const cacheUrl = process.env.APP_URL || 'http://localhost:3021';
const http = axios.create({
	baseURL: cacheUrl,
	headers: { 'Cache-Control': 'no-cache' },
	adapter: axiosExt.throttleAdapterEnhancer(axios.defaults.adapter, { threshold: 500 })
});
...
async function getBlockchainOverview(blockchain) {
  ...
  const response = await http.get(overviewUrl);
...
export default async function (ctx, blockchain) {
  ...
  await Promise.all([
    ...
    ctx.$store.commit('pools/setTaPeriods', taPeriods),
