...
import { poolNameWithoutAddr } from './utils.mjs';
...
export function getPoolsPeriodDepthAndVolumeUsd(poolHistoryDepths, poolHistorySwaps, periodKey) {
  ...
  const startAtUnixTime = df.getUnixTime(new Date()) - secondsElapsedSincePeriod;
  ...
  return Object.keys(poolHistoryDepths).map((poolId, index) => {
    ...
    const totalVolumeUsd = swapIntervals.reduce((acc, next) => {
...
export function technicalAnalysis(
  ...
  const periodResolutionKey = ['period24H', 'period1W', 'period1M'].find(k => k === periodKey) ? 'period1HM' : 'period1Y';
  const secondsElapsedSincePeriod = periodKeyToSecondsMap[periodKey];
  const startTime = df.getUnixTime(new Date()) - secondsElapsedSincePeriod;
...
function getCorrellationsForUnixtime(
  ...
  const periodKeyToStartTimestamp = {
    period24H: df.getUnixTime(df.subHours(df.fromUnixTime(atUnixtime), 24)), // in hours
    period1W: df.getUnixTime(df.subHours(df.fromUnixTime(atUnixtime), 24 * 7)), // in hours
    period1M: df.getUnixTime(df.subDays(df.fromUnixTime(atUnixtime), 24 * 30)), // in hours
    period3M: df.getUnixTime(df.subDays(df.fromUnixTime(atUnixtime), 90)),
    period6M: df.getUnixTime(df.subDays(df.fromUnixTime(atUnixtime), 180)),
    period1Y: df.getUnixTime(df.subDays(df.fromUnixTime(atUnixtime), 365)),
  };
